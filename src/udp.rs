use crate::{tcp::*, Error, IntoTargetAddr, Result, TargetAddr, ToProxyAddrs};
...
pub struct Socks5UdpCodec(BytesMut);

impl Socks5UdpFramed {
    pub async fn connect<'t, P, T1, T2>(proxy: P, target: T1, bind_addr: Option<T2>) -> Result<Self>
    where
        P: ToProxyAddrs,
        T1: IntoTargetAddr<'t>,
        T2: ToSocketAddrs,
    {
        ...
        let codec = Socks5UdpCodec::new(&target);
        ...
        Ok(Self { framed, stream })
    }

    pub async fn connect_with_password<'a, 't, P, T1, T2>(
        proxy: P,
        target: T1,
        bind_addr: Option<T2>,
        username: &'a str,
    ...
    pub fn target_addr<'a>(&'a self) -> TargetAddr<'a> {
...
impl Sink<(Bytes, SocketAddr)> for Socks5UdpFramed {
    type Error = <Socks5UdpCodec as Decoder>::Error;

    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<StdResult<(), Self::Error>> {
        self.project().framed.poll_ready(cx)
    }

    fn start_send(self: Pin<&mut Self>, item: (Bytes, SocketAddr)) -> StdResult<(), Self::Error> {
...
impl Socks5UdpCodec {
    pub fn new<'a>(target: &'a TargetAddr) -> Self {
...
impl Encoder<Bytes> for Socks5UdpCodec {
    type Error = Error;

    // TODO: consider fragment
    fn encode(&mut self, data: Bytes, buf: &mut BytesMut) -> StdResult<(), Self::Error> {
        buf.clear();
        buf.extend(&self.0);
        buf.extend(data);
        Ok(())
